...
class Command(BaseCommand):
    help = 'Rollback the latest migration (with dependencies) and backup'
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"No migrations found for app: {app_name}"))
            return

        # Collect apps in reverse dependency order
        rollback_apps = []
        for app_label, migration_name in rollback_plan:
            if app_label not in rollback_apps:
                rollback_apps.append(app_label)
        rollback_apps = rollback_apps[::-1]  # reverse order for rollback
